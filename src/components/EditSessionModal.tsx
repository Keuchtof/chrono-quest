...
export default function EditSessionModal({ open, session, blocs, settings, onSave, onAddTag, onClose }: Props) {
  ...
  function handleSave() {
    if (isRest) {
      ...
      onSave({ blocId, tag, config: config || 'Journée', posture: '', zone: '',
        duration: 0, date, startTime: startTs, endTime: startTs, chargeNiveau: 0 })
      onClose()
      return
    }
