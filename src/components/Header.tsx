...
import { formatMonthYear, getMonthSessions, secondsToDisplay, getBalance, getMonthStart, getDateStr, formatBalance } from '../utils'
...
export default function Header({ store, now }: Props) {
  ...
  const monthStart = getMonthStart(today.getFullYear(), today.getMonth())
  const monthBalance = getBalance(store.sessions, store.settings, monthStart, todayStr, store.activeTimer, now)
      ...
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="text-right">
          <p className="text-base font-bold text-gray-900 leading-tight">
            {secondsToDisplay(totalSecs)}
            <span className="text-sm font-normal text-gray-400"> / {secondsToDisplay(objectiveSecs)}</span>
          </p>
          <p className="text-xs font-bold mt-0.5"
            style={{ color: monthBalance >= 0 ? '#22C55E' : '#EF4444' }}>
            {formatBalance(monthBalance)} ce mois
          </p>
        </div>
      </div>
