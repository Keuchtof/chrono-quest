...
export default function JourTab({ store, now }: Props) {
  ...
  const blocStats = store.blocs.map(b => {
    ...
    return { bloc: b, totalSecs, count, avgSecs }
  ...
  const progress = dailyObj > 0 ? Math.min(dayTotal / dailyObj, 1) : 0
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mt-1.5 mb-3">
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${progress * 100}%`, background: 'linear-gradient(to right, #3B82F6, #22C55E)' }} />
            </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap mb-1">
                      ...
                      {s.config  && <SessionChip label={s.config}  color="#3B82F6" />}
                      {s.posture && <SessionChip label={s.posture} color="#8B5CF6" />}
                      {s.tag     && <SessionChip label={s.tag}     color="#6B7280" />}
                    </div>
                    <span className="text-sm font-semibold text-gray-900">{formatDuration(s.duration)}</span>
                  </div>
                  <button onClick={() => setEditSession(s)}
                    className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 rounded-lg">✏️</button>
                  <button onClick={() => store.deleteSession(s.id)}
                    className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-600 rounded-lg">🗑</button>
                </div>
              </div>
            ))}
          </div>
...
function SessionChip({ label, color }: { label: string; color: string }) {
  return (
